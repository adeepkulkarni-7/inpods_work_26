"""
Core Audit Engine - Integration Package
Dimension-agnostic curriculum mapping using Azure OpenAI

This is the main AI engine for curriculum mapping, designed for easy integration
...
class AuditEngine:
    ...
    Designed as a tool provider for AI agent integration.
    ...
    Usage:
        ...
        ratings = engine.rate_existing_mappings(
    ...
    def __init__(self, config):
        ...
        Args:
            config (dict): {
                'api_key': str,
                'azure_endpoint': str,
                'api_version': str (default: '2024-02-15-preview'),
                'deployment': str (default: 'gpt-4')
            }
        """
        self.config = config
        self.client = None
    ...
    def test_connection(self):
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.config.get("deployment", "gpt-4"),
    ...
    def _load_reference_data(self, reference_csv, dimension):
        ...
        if dimension == 'area_topics':
            ...
            for _, row in df.iterrows():
                ...
                if 'ID' in df.columns:
                    id_val = row.get('ID')
                    type_val = row.get('Type', row.get('Category', ''))
...
"""
    ...
    def apply_and_export(self, question_csv, recommendations, selected_indices, dimension, output_folder):
        ...
        Args:
            question_csv (str): Path to original question file
...
class LibraryManager:
    """
    Manages persistent storage of mapping sets (file-based).

    For database storage, use the DatabaseManager from integration.database
    """
